...
class popular_manga_repo(private val searchQuery: String? = null) : PagingSource<Int, Manga>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Manga> {
        ...
        return try {
            ...
            val mangaList = apiResponse?.data?.Page?.media?.mapIndexed { index, manga ->
                manga.copy(
                    calculatedRank = (page - 1) * perPage + index + 1
                )
            }?.distinct() ?: emptyList()
            LoadResult.Page(
                data = mangaList,
